...
import { MdDelete, MdCreate, MdCheckCircle, MdHighlightOff} from 'react-icons/md';
  ...
  useEffect(() => {
    ...
    async function listEnrollment() {
      ...
      const data = response.data.map(enrollment => ({
        ...enrollment,
        startFormatted: format(parseISO(enrollment.start_date),
        "dd 'de' MMMM 'de' yyyy",
        {locale: pt }
        ),
        endFormatted:format(parseISO(enrollment.end_date),
        "dd 'de' MMMM 'de' yyyy",
        {locale: pt }
        ),
      }));
